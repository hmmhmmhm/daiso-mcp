...
이 디렉터리는 운영에 쓰는 스크립트와 외부 서비스 분석용 리서치 스크립트를 함께 보관합니다.

## 운영 스크립트

- `mcp-smoke.ts`: 배포된 MCP 엔드포인트에서 대표 도구 목록과 호출 결과를 확인합니다.
- `cli-smoke.ts`: 빌드된 CLI가 주요 사용자 시나리오를 처리하는지 확인합니다.
- `generate-openapi.ts`: API 라우트 기반 OpenAPI 산출물을 생성합니다.
- `workers-chart-data.ts`, `update-workers-invocations-chart.ts`: Cloudflare Workers 호출량 차트를 갱신합니다.
