...
if [[ -z "$OUT_DIR" ]]; then
  TS="$(date +%Y%m%d-%H%M%S)"
  OUT_DIR="captures/gs25-pgl-meta-${TS}"
fi

mkdir -p "$OUT_DIR"
RAW_LOG="${OUT_DIR}/frida-pgl-meta-raw.log"
JSONL="${OUT_DIR}/gs25-pgl-meta-events.jsonl"
...
"${FRIDA_CMD[@]}" \
  | tee "$RAW_LOG" \
  | awk '/\[GS25_PGL_META\]/ { idx=index($0,"{"); if (idx>0) print substr($0, idx); fflush(); }' \
  | tee -a "$JSONL"
