...
4. 결과는 핵심 정보 중심으로 보기 쉽게 정리
   ...
   - MCP/REST 응답에 `imageUrl`이 있으면 해당 이미지를 반드시 마크다운 이미지로 렌더링하고, 이미지가 여러 개이면 각 항목의 이미지를 전부 표시하며, 사용자가 따로 요청하기 전까지 임의로 생략하지 않음

## 주의사항

- 다이소 재고 확인 전에는 반드시 제품 검색으로 ID를 확인
- 다이소 재고 조회는 `productId` + `keyword` 또는 위치(`lat`,`lng`)만으로 가능하며 `storeCode`가 필요하지 않음
- 상품/재고 응답에 `imageUrl`이 있으면 답변에서 반드시 마크다운 이미지로 함께 표시하고, 여러 개면 첫 번째만 남기지 말고 전부 표시함
