...
- 무료 직접 요청이 400/403/429로 실패할 때 원본 HTTP 오류를 보존합니다. CU 웹 매장 조회와 GS25 검색의 별도 대체 경로도 원본 오류를 보존하고, CGV 차단 진단에는 원본 상태 코드를 포함합니다. 기존 Zyte 키가 설정되어 있어도 비용 정책 메시지로 원본 오류를 덮지 않습니다. 유료 Zyte 호출 차단은 유지합니다.
