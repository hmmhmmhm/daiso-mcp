...
- CU 매장 검색은 원본 웹 요청이 차단될 때 기존 Zyte 폴백으로 복구되지만, 재고 검색은 원본 JSON API의 `400 Request Blocked`를 그대로 반환한다.
...
5. 폴백 자체가 실패하면 Zyte 오류를 숨기지 않고 서비스·원인을 식별할 수 있는 메시지로 전달한다.
